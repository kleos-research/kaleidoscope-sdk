...
This repository is a clean public control-plane implementation for a separately supplied native `kscope` engine. It contains no memory engine, MCP proxy, account service, model, vault implementation, or private engine source. The release-candidate package names are `kscope-memory` (Python) and `@kleos-research/kaleidoscope` (TypeScript); this checkout remains local-only staging until the protected license, signing, registry, and publication gates are approved.
...
`kaleidoscope hook session-start [--profile NAME] [--no-memories]` is invoked **by** Claude Code, not by users. It **probes, then reports**, and it exits 0 on every path -- a hook that exits non-zero is a hook the user turns off.

It does four bounded things and publishes what each one found:

* reads the `mcpServers.kaleidoscope` entry that applies to this project, and speaks MCP over stdio to **that** command -- `initialize`, then `tools/list` -- so the report is about the server the harness actually starts;
* runs `kscope profile launch`, which is ungated, to separate "the memory configuration is broken" from "the configuration is fine and the client rejected the tool list";
* asks the engine `where --root-only` for the project, which redirects a linked worktree to its main checkout;
* retrieves a few memories for that project and injects them.

The first line of the emitted context is a one-line JSON object carrying `tools_visible` and every rung the verdict was computed from, so a later session can be **audited** rather than believed. When the tools are not reachable it says so plainly and names the CLI fallback, which is unaffected by an MCP fault.

It **does** call `search`, and that is a reversal of an earlier decision. `search` writes an exposure row on every call and `ledger: false` is refused, so retrieving at all costs one exposure row per session start. That is now an accepted, bounded cost -- `top_k` 4 and a `maximum_context_bytes` ceiling -- and `--no-memories` or `KALEIDOSCOPE_HOOK_MEMORIES=0` turns retrieval off without turning the hook off. It never calls `remember`: the hook reads, and a test asserts no write reaches the engine.

Both budgets are stated rather than implied. Bytes: the whole JSON line is capped at 4 KiB and the context inside it at 2,600, of which the memories may take 1,500; an over-long context is **shrunk**, never replaced with a generic sentence. Time: every subprocess runs under a per-stage timeout and one shared 6 s deadline, well inside the 10 s the settings entry allows. Measured end to end against the real engine and a 179-memory vault: **~100 ms**.

It earns its place over `CLAUDE.md` alone because `CLAUDE.md` is read once at session start, while the hook fires again on `resume`, `clear` and `compact` -- so the context survives compaction.
...
**User scope is the default.** The MCP entry and the `SessionStart` hook go under your home directory, so they are visible from any directory -- including a git worktree and a temporary clone, which is where an agent harness usually puts your work and where a project-scoped entry is not read at all. `--scope project` puts them in the project instead, and every command says on stderr which scope it used and whether that came from the flag or the default.

Instructions and the skill (`CLAUDE.md`, `AGENTS.md`, the Cursor rule, `SKILL.md`) are **always** written to the project, whatever `--scope` says: git is the transport that carries them into a worktree or a clone, and a home-wide `CLAUDE.md` would inject Kaleidoscope into every unrelated project you open. The JSON reports this as `scope_applies_to` and `instructions_scope` so the split is never a surprise.

Project-scoped files land at the **project root**, not the working directory. The root is resolved by the engine (`kscope where --root-only`), which walks up for a marker such as `CLAUDE.md`, `.claude/` or `.git`; `--project PATH` overrides it. Inside a linked worktree the files go into the *worktree*, not the main checkout.

If you already ran the old project-scope default, a bare `init` leaves those files exactly where they are and reports a `project_scope_carryover` warning naming them and the `teardown` that removes them. Nothing is removed implicitly.

Every mutation is previewed, requires confirmation unless `--yes` is supplied, takes an atomic snapshot check, writes a bounded sibling backup for an existing file, and records exact ownership in a sibling receipt.

**Content that is already byte-identical to what the manager writes is adopted, not refused.** Placing the skill or the MCP entry by hand -- following this document -- used to be a hard stop, because the discriminator was the receipt rather than the content; now the manager writes the receipt beside your file and changes no bytes. If the content *differs* it still refuses, and the refusal names the flag that proceeds (`--no-connect`, `--no-instructions`, `--no-skill`, `--no-hooks`) or the exact key to delete by hand. Teardown of an adopted **entry** removes the entry; teardown of an adopted **file** leaves the file and removes only the receipt, because that file is yours.

Exit codes: `0` success (including every documented no-op), `2` any refusal, `3` `doctor` completing with at least one check reporting an issue.
...
The Python distribution is `kscope-memory`; the TypeScript distribution
...
The two clients get the engine differently, and deliberately. The npm package
carries a natively tested platform companion -- on macOS arm64 that is
`@kleos-research/kaleidoscope-darwin-arm64` -- and installs the `kaleidoscope`
and `kscope` commands from it. No postinstall step and no runtime download.

The Python package carries nothing. It finds a `kscope` that is already on the
machine, in this order: a path you pass, then `KALEIDOSCOPE_ENGINE`, then the
directory this Python installs its own commands into, then `PATH`. It used to
require a platform wheel instead, and that wheel was a mandatory dependency
that nothing built and no index carried, so the package could not be installed
at all. When nothing is found, the error names every place it looked and the
